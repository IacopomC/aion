...
//	if (debug) ROS_DEBUG("Command received %s %s\n",goal->operation.c_str(),goal->id.c_str());
	...
	else if (goal->operation == "predict")
	{
		if (frelementArray == NULL)
		{
			mess << "Cannot predict - no data has been added to the model yet.";
			result.message = mess.str();
			result.success = -1;
			server->setAborted(result);
			return;
	...
	else if (goal->operation == "entropy")
	{
		if (frelementArray == NULL)
		{
			mess << "Cannot compute entropy - no data has been added to the model yet.";
			result.message = mess.str();
			result.success = -1;
			server->setAborted(result);
			return;
...
int main(int argc,char* argv[])
{
	...
	// Get action server name from parameter (default to "/fremenarray" for backwards compatibility)
	std::string action_server_name;
	private_nh.param("action_server_name", action_server_name, std::string("/fremenarray"));
	
	ROS_INFO("Starting FremenArray action server at: %s", action_server_name.c_str());
	server = new Server(*n, action_server_name, boost::bind(&actionServerCallback, _1, server), false);
	server->start();
